...
(function(){
  ...
  function render(d){
    ...
    var fcSource=(d.fcSource||'offline').toString();
    var fcPresentation=fcSource==='presentation';
    var fcReal=fcSource==='real_msp'&&!!d.fcRealOnline;
    var fcStatusOk=fcPresentation||(fcReal&&!!d.fcStatusValid);
    var fcAttitudeOk=fcPresentation||(fcReal&&!!d.fcAttitudeValid);
    var fcRcOk=fcPresentation||(fcReal&&!!d.fcRcValid);
    var fcBatteryOk=fcPresentation||(fcReal&&!!d.fcBatteryValid);
    var fcAltitudeOk=fcPresentation||(fcReal&&!!d.fcAltitudeValid);
    setChip('fcChip',fcReal?'ok':'warn',fcPresentation?'演示数据':(fcReal?'MSP 已连接':'飞控未连接'));
    setText('fcSub',fcPresentation?'模拟飞控 · 非真实硬件':(fcReal?('接收 '+num(d.fcMspRxBytes,0)+' 字节 / 数据 '+ms(d.fcDataAgeMs)):'等待 UART3 MSP'));
    setText('roll',fcAttitudeOk?deg(d.roll):'--');
    setText('pitch',fcAttitudeOk?deg(d.pitch):'--');
    setText('yaw',fcAttitudeOk?deg(d.yaw):'--');
    setText('armed',fcStatusOk?(d.armed?'已解锁':'已锁定'):'--');
    setText('cycle',fcStatusOk?num(d.fcCycleTimeUs,0)+' us':'--');
    setText('vario',fcAltitudeOk?cm(d.fcVario)+'/s':'--');
    el('horizon').style.transform='translateY('+(fcAttitudeOk?Math.max(-34,Math.min(34,Number(d.pitch)||0))*1.2:0)+'px) rotate('+(fcAttitudeOk?-(Number(d.roll)||0):0)+'deg)';
    setText('rcRoll',fcRcOk?num(d.rcRoll,0):'--');
    setText('rcPitch',fcRcOk?num(d.rcPitch,0):'--');
    setText('rcThr',fcRcOk?num(d.rcThrottle,0):'--');
    setText('rcYaw',fcRcOk?num(d.rcYaw,0):'--');
    rcBar('barRoll',fcRcOk?d.rcRoll:1500);
    rcBar('barPitch',fcRcOk?d.rcPitch:1500);
    rcBar('barThr',fcRcOk?d.rcThrottle:1000);
    rcBar('barYaw',fcRcOk?d.rcYaw:1500);
    setText('aux',fcRcOk?('A1 '+num(d.rcAux1,0)+' / A2 '+num(d.rcAux2,0)+' / '+num(d.rcChannelCount,0)+'通道'):'--');
    setText('msp',fcPresentation?'演示链路':('发 '+num(d.fcMspTxFrames,0)+' / 收 '+num(d.fcMspRxBytes,0)+' / 超时 '+num(d.fcMspTimeouts,0)));
    ...
    setText('batMain',fcBatteryOk&&finite(d.batV)?Number(d.batV).toFixed(2)+' V':'--');
    setText('batCells',fcBatteryOk&&d.batCells?num(d.batCells,0)+'S':'--');
    setText('altitude',fcAltitudeOk?cm(d.baroAlt):'--');
    setText('outputMode',fcPresentation?'演示模式':(d.fcRealOutputCompiled?'辅助固件':'只读遥测'));
